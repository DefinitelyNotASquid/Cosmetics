...
class CosmeticsSkinCommand implements CommandExecutor {
    /**
     ...
     */
    CosmeticsSkinCommand(Cosmetics plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
                ...
                {
                    sendPropMessage(sender, "command.not_skinable");
                    return true;
                }

                else {
                    String type = CosmeticsHandler.stripString(((Player) sender).getInventory().getItemInMainHand().getType());
                    ((Player) sender).openInventory(plugin.cosmeticsChatGui.getInventory(0, type));
                }
